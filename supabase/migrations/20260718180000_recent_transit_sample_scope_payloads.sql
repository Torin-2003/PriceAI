create or replace function public.list_recent_api_transit_availability_sample_scopes(
  p_station_ids text[],
...
as $$
  ...
  select
    ...
    jsonb_agg(
      ...
      order by recent.checked_at asc
    ) as samples
...
revoke all on function public.list_recent_api_transit_availability_sample_scopes(text[], integer, timestamptz)
  from public, anon, authenticated;
grant execute on function public.list_recent_api_transit_availability_sample_scopes(text[], integer, timestamptz)
  to service_role;

comment on function public.list_recent_api_transit_availability_sample_scopes(text[], integer, timestamptz) is
  'Returns one row per current transit monitoring scope with up to 60 indexed recent samples in JSON.';

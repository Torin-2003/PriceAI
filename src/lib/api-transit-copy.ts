...
export const TRANSIT_RATE_BREAKDOWN_EXPLANATION =
  "这里拆开展示综合倍率的来源。充值倍率表示充值后人民币折成站内额度的成本：1:1 记为 1.00x，1:10 记为 0.10x，也就是大约 0.10 元买到 1 单位额度。模型倍率表示该模型分组相对官方价格的扣费倍数。两者相乘后得到左侧综合倍率。例：充值倍率 0.10x × 模型倍率 1.50x = 综合倍率 0.15x。";

export const TRANSIT_RECHARGE_COEFFICIENT_EXPLANATION =
  "充值倍率只描述人民币与站内额度的换算关系，不代表某个模型本身便宜或贵。例：1:1 记为 1.00x，1:10 记为 0.10x，表示约 0.10 元换到站内 1 单位额度；最终价格还要再乘以模型倍率。";

...
| 基础设施容量、异常流量与成本治理 | P1 实施中 | P0 只读归因已完成；P1 已核对现有 Cloudflare 单 IP Block、全站型 Rate Limiting 和 Free 配额，并完成 Next.js / OpenNext 实施设计 | 采用 Durable Object Queue 修复 revalidation；主导航改为意图预取，长列表关闭 viewport prefetch；availability 按 8 天设计 rollup 和 dry-run retention，暂不执行生产删除 | 先完成 DO Queue / prefetch 的本地验证和独立提交，再实现 rollup / retention migration；Cloudflare 保存规则、迁移应用和数据删除前保留确认门 | [基础设施容量、异常流量与成本治理规划](archive/in-progress/product/2026-07-14_priceai-infrastructure-capacity-traffic-and-cost-governance-plan.md)、[Cloudflare 24 小时归因](archive/in-progress/product/2026-07-14_priceai-cloudflare-24h-traffic-attribution-baseline.md)、[Supabase 安全清理策略](archive/in-progress/product/2026-07-14_priceai-supabase-retention-and-cleanup-batch-strategy.md) |
...
| P4 基础设施和成本优化 | P1 实施中 | 24 小时基线已完成；Supabase 8 天留存有约 436k 清理候选，Cloudflare Top 10 路径占约一半请求和 CPU，R2 Class B 约 2.49M / 24h，Observability 约 72k error-level events | 先落地 DO Queue、prefetch 收敛和 rollup / dry-run retention；生产 WAF / Rate Limiting、日志采样、迁移应用与删除数据均不直接执行；见[基础设施容量、异常流量与成本治理规划](archive/in-progress/product/2026-07-14_priceai-infrastructure-capacity-traffic-and-cost-governance-plan.md) |
...
| 优先级 | 待办 | 为什么重要 | 建议下一步 |
| --- | --- | --- | --- |
| P1 | 完成基础设施治理第一批应用侧与数据侧实现 | P0 已证实 availability 8 天留存能处理约 436k 行，而 14 天只有约 7.6k；Cloudflare 侧少数路径、RSC 预取、OpenNext revalidation error 和 R2 Class B 是当前主要成本来源 | DO Queue 与 prefetch 收敛已进入本地验证；下一步实现 rollup / 默认 dry-run retention migration。生产规则保存、迁移应用和删除数据前仍需单独确认 |
